...
| Android OCR 후보 영역 과다 | 화면 전체를 OCR 대상으로 삼으면 속도, 배터리, 개인정보, 오탐 위험이 커지고 썸네일/카드 전체가 다시 가려질 수 있음 | YouTube grid/Shorts 캡처, BBA-79, `contentDescription` only 카드 제약 | OCR은 이미지 내부 텍스트를 보완할 수 있지만 전체 화면 실시간 OCR은 모바일 foreground UX에 맞지 않음 | `VisualTextRoiPlanner`를 추가해 접근성 text node 마스킹과 별도로 상단 visible composite card/visual node 최대 4개만 OCR 후보로 선별 | `VisualTextRoiPlannerTest`, `:app:testDebugUnitTest` | 실제 OCR 엔진 연결, crop bitmap 생성, backend span 투영 latency는 다음 루프에서 검증 |
| Android OCR 후보 진단 누락 | YouTube 화면에 이미지/썸네일 텍스트 후보가 있어도 일반 텍스트 분석 후보가 0개면 최근 진단에 아무 것도 남지 않음 | YouTube grid/Shorts 테스트 화면, `comments.isEmpty()` 조기 종료 경로 | OCR 엔진이 아직 연결되지 않은 상태에서 전체 썸네일을 임의로 가리면 오탐과 위치 오차가 커짐 | 일반 텍스트 후보가 없어도 ROI 후보가 있으면 최근 진단에 OCR 후보 수와 상위 ROI 좌표를 저장 | `:app:testDebugUnitTest`, `:app:assembleDebug` | 실제 OCR 실행/마스킹은 별도 spike에서 latency와 배터리 비용을 확인해야 함 |
| Android ROI OCR 1차 연결 | 접근성 text node가 없는 YouTube 썸네일/이미지 텍스트는 backend에 전달할 문자열이 없어 계속 미탐됨 | YouTube grid/Shorts 캡처, `VisualTextRoiPlanner`, `android:canTakeScreenshot` capability | 전체 화면 OCR은 느리고 개인정보/배터리 부담이 크며, OCR이 판정자가 되면 backend 모델 기준이 흐려짐 | 상단 ROI 최대 2개만 screenshot crop 후 ML Kit Korean/Latin OCR을 돌리고, OCR line을 기존 `/analyze_android` 후보로 변환. 일반 접근성 분석이 마스킹 가능한 span을 못 만든 경우에도 같은 ROI를 후속 보정으로만 실행 | `:app:testDebugUnitTest`, `:app:assembleDebug` | ADB 기기 연결이 끊겨 실기기 재설치는 다음 루프에서 재확인 필요 |
...
| Android 범용 앱 후보 수집 누락 | Chrome/일반 앱 화면의 텍스트가 접근성 트리에는 있어도 backend 분석 후보로 변환되지 않음 | Google/Chrome 화면 캡처, YouTube 전용 extractor 의존 구조, 접근성 service package 제한 | YouTube 전용 OCR 기능이 아니라 화면에 보이는 텍스트를 앱 공통 수집 계층으로 다뤄야 함 | `ScreenTextCandidate` 모델을 추가하고 YouTube/Instagram/TikTok 기존 extractor는 감싸서 유지, 그 외 앱은 generic accessibility candidate로 backend에 전달 | `ScreenTextCandidateExtractorTest`, `:app:testDebugUnitTest`, `:app:assembleDebug`, APK 재설치 | OCR 기반 exact geometry 보정은 별도 단계이며, 현재는 접근성 bounds 기반 후보 확장까지 완료 |
| Android 브라우저 접근성 좌표 오차 | 모바일 Chrome 검색 결과/AI 개요/관련 검색어에서 마스크가 단어 위치가 아니라 행 위에 떠다님 | 사용자 캡처, `android-accessibility-range:*` 추정 후보, 브라우저 접근성 bounds가 행/카드 단위로 제공되는 현상 | 접근성 텍스트는 문맥 분석에는 유용하지만 단어 단위 좌표로는 불충분하며, 잘못된 좌표를 빠르게 렌더링하면 실시간성보다 오작동 체감이 커짐 | 브라우저 generic 후보를 `android-accessibility-browser:*` 출처로 분리하고, 브라우저에서는 추정 range 후보를 만들지 않으며 넓은 browser accessibility mask를 renderer에서 제외 | `ScreenTextCandidateExtractorTest`, `MaskOverlayPlannerTest`, `VisualTextOcrCandidateFilterTest` | 일부 브라우저 row는 OCR geometry 보정 전까지 마스킹되지 않을 수 있음 |
| Android 브라우저 compact bounds 과신 | AI 개요/검색 결과에서 120~200px 정도로 작아 보이는 bounds도 실제 단어 좌표가 아니라 snippet 일부/행 좌표라 마스크가 떠다님 | 모바일 Chrome 캡처, `android-accessibility-browser:*` renderer threshold, compact browser regression test 비교 | 잘못된 마스크는 누락보다 UX 신뢰도를 더 크게 떨어뜨리므로, 브라우저 접근성 renderer는 “정확해 보이는 아주 작은 후보”만 통과시켜야 함 | 브라우저 접근성 렌더 허용 폭/높이/원문 길이를 더 좁히고, loose browser snippet bounds를 거부하는 회귀 테스트 추가 | `MaskOverlayPlannerTest`, `:app:testDebugUnitTest`, `:app:assembleDebug` | 일부 짧은 브라우저 텍스트도 OCR geometry 보정 전까지 보일 수 있음 |
| Android 스크롤 후 재부착 지연 | 스크롤 중 mask를 지우는 정책은 맞지만, 안정화 대기 시간이 길어 화면이 멈춘 뒤에도 마스크가 늦게 붙음 | 사용자 실기기/에뮬레이터 캡처, scroll delay/stabilization constant 확인 | stale mask를 막기 위해 scroll 중 OCR/접근성 마스크를 지우되, stable 판단은 너무 보수적이면 실시간성이 떨어짐 | scroll parse delay와 stabilization window를 축소해 화면 정지 후 재분석을 더 빨리 시작하도록 조정 | `:app:testDebugUnitTest`, `:app:assembleDebug` | 너무 빠른 재분석은 일부 앱에서 아직 움직이는 bounds를 잡을 수 있어 renderer sanity check와 함께 유지해야 함 |
| Android browser/generic visual 렌더링 차단 | Chrome 모바일에서 검색창/AI 개요/관련 검색어 주변에 작은 검은 막대가 떠다니고, generic OCR 후보가 스크롤 중 다른 위치에 남음 | 사용자 캡처, `android-accessibility-browser:*`, `screen:accessibility_text:*`, `ocr:generic-visual-region:*` 출처 비교 | backend가 유해 판단을 하더라도 renderer가 신뢰할 수 있는 단어 좌표를 갖지 못하면 잘못 가리는 것이 더 큰 UX 결함임 | browser accessibility, generic screen accessibility, generic visual region 후보는 분석/진단 후보로 유지하되 overlay 렌더링은 금지하고, exact range 또는 앱별 OCR projection이 있는 후보만 그리도록 게이트 강화 | `MaskOverlayPlannerTest`, `:app:testDebugUnitTest`, `:app:assembleDebug`, APK 재설치 | Chrome/일반 앱에서 일부 유해어는 exact geometry 보정이 구현될 때까지 보일 수 있음 |
