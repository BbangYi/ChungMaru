...
/** The original parser from the standalone YouTubeParser project. */
object YoutubeCommentExtractor {
    ...
    fun extractComments(nodes: List<ParsedTextNode>): List<ParsedComment> {
        ...
        return result.distinctBy {
            val authorId = it.authorId.orEmpty().lowercase()
            if (authorId.isNotBlank()) {
                "$authorId|${it.commentText.trim()}"
            } else {
    ...
    private fun normalizeAuthor(text: String): String? {
        return text.trim().removePrefix("@").ifBlank { null }
    }
