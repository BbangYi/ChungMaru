...
- 유해 이미지 차단은 현재 viewport의 `img`, `picture`, `video`, `background-image` 후보를 먼저 보고, 위험 URL/domain/text 신호가 있는 linked media grid는 제한된 범위에서 보강 수집합니다. 이미지/영상이 늦게 로드되면 load/metadata 이벤트를 통해 재스캔합니다.
