...
PROFANITY_PATTERNS = [
    # ── 비속어 (profanity) ──────────────────────────────
    # 씨발 계열
    (r"씨[이]*발|시[이]*발|씨[이]*팔|시[이]*팔|씨[이]*벌|ㅅ[\s\W_]*ㅂ|ㅆ[\s\W_]*ㅂ|씨[0-9]*발|시[0-9]*발|싸[이]?발|쒸[이]?발|씌발|c발|씨bal|니미씨|니미시|시불|시블|씨부[랄럴렁]|시부[랄럴렁]|s{1,2}[\W_]*(?:h[\W_]*)?i[\W_]*b[\W_]*a[\W_]*l|tlqkf", "씨발", "profanity"),
    # 개새끼 계열
    (r"개[새세][끼키]|개[새세]끼|개[새세]기|개색[끼키]|개[쌔쎄][끼키]|개[0-9]*새끼|ㄱㅅㄲ|gae[\W_]*s(?:ae|e|a)[\W_]*k{1,2}i|rotori", "개새끼", "profanity"),
    # 병신 계열
    (r"병[\.]*신|by[eou]+ng[\W_]*s?in|q[\W_]*u[\W_]*d[\W_]*t[\W_]*l[\W_]*s|qudtkf|ㅂ[\s\W_]*ㅅ|병[0-9]*신|빙신|벼ㅇ신|병딱|병1신", "병신", "profanity"),
    # 지랄 계열
    (r"지[이]*랄|ㅈ[\s\W_]*ㄹ|지[0-9]*랄|jiral|wlfkf", "지랄", "profanity"),
    # 좆/좃 계열
    (r"좆|좇|조[까깟]|졸라|존나|ㅈ[\s\W_]*ㄴ|존[0-9]*나|좃|좋까|존[나]?맛|좆같|jonna|whssk", "좆", "profanity"),
    # 씹 계열
    ...
    # 새끼 단독
    (r"새[끼키]|색[끼키]|쌔[끼키]|ㅅ[\s\W_]*ㄲ", "새끼", "profanity"),
    # 엿/엿먹 계열
    ...
    # Google 검색 결과/광고성 문구에서 붙어 나오는 성인 유흥 서비스 표현.
    # "성인" 단독은 정상 맥락이 많으므로 결합어만 잡는다.
    (r"콜[\s\W_]*걸|성인[\s\W_]*(?:마사지|업소)|출장[\s\W_]*(?:마사지|안마|만남)|유흥[\s\W_]*(?:업소|마사지|주점)|안마[\s\W_]*방|키스[\s\W_]*방|립[\s\W_]*카페|룸[\s\W_]*(?:싸롱|살롱)|셔츠[\s\W_]*룸|조건[\s\W_]*만남|성[\s\W_]*매매", "성인유흥", "sexual"),
